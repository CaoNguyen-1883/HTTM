...
import {
  ImageUpload,
  UploadedImage,
} from "../../components/seller/ImageUpload";
...
export const SellerProductEditPage = () => {
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.images && product.images.length > 0) {
        const loadedImages: UploadedImage[] = product.images.map(
          (img: any) => ({
            url: img.imageUrl,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        "Product updated successfully! It will be reviewed again before going live.",
      );

      // Preserve the page and filter parameters when navigating back
      const returnUrl = (location.state as any)?.from || "/seller/products";
      navigate(returnUrl);
    } catch (error: any) {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <button
          onClick={() => {
            const returnUrl =
              (location.state as any)?.from || "/seller/products";
            navigate(returnUrl);
          }}
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Update your product details. Product will be reviewed again after
            update.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-800">
              <strong>Note:</strong> Updating images will replace all existing
              product-level images. Variant-specific images are not affected.
            </p>
          </div>
        </div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-sm text-yellow-800">
            <strong>Note:</strong> After updating, your product will be set to
            PENDING status and require admin approval before going live again.
          </p>
        </div>
