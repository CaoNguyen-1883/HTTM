...
@Tag(name = "Product Management", description = "APIs for managing products with variants and approval workflow")
public class ProductController {
    ...
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> getAllProducts(
        ...
        if (keyword != null || categoryId != null || brandId != null || minPrice != null || maxPrice != null) {
            products = productService.filterProducts(keyword, categoryId, brandId, minPrice, maxPrice, pageable);
        } else {
    ...
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> searchProducts(
            @Parameter(description = "Search keyword", example = "gaming mouse")
            @RequestParam String keyword,
            @PageableDefault(size = 20) Pageable pageable) {
        Page<ProductSummaryDto> products = productService.searchProducts(keyword, pageable);
    ...
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> getProductsByBrand(
    ...
    @GetMapping("/trending")
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> getTrendingProducts(
            @PageableDefault(size = 10) Pageable pageable) {
        Page<ProductSummaryDto> products = productService.getTrendingProducts(pageable);
    ...
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> getBestSellingProducts(
            @PageableDefault(size = 10) Pageable pageable) {
        Page<ProductSummaryDto> products = productService.getBestSellingProducts(pageable);
    ...
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> getTopRatedProducts(
            @PageableDefault(size = 10) Pageable pageable) {
        Page<ProductSummaryDto> products = productService.getTopRatedProducts(pageable);
    ...
    public ResponseEntity<ApiResponse<List<ProductSummaryDto>>> getSimilarProducts(
            @PathVariable UUID id,
            @Parameter(description = "Number of similar products to return")
            @RequestParam(defaultValue = "5") int limit) {
        List<ProductSummaryDto> products = productService.getSimilarProducts(id, limit);
        return ResponseEntity.ok(ApiResponse.success("Similar products retrieved successfully", products));
    }
    ...
    public ResponseEntity<ApiResponse<ProductDto>> createProduct(
        ...
        ProductDto created = productService.createProduct(request, userDetails.getId());
        return ResponseEntity.status(201)
                .body(ApiResponse.created("Product created successfully and pending approval", created));
    }
    ...
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> getMyProducts(
            ...
            @PageableDefault(size = 20, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
        CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
        Page<ProductSummaryDto> products = productService.getSellerProducts(userDetails.getId(), pageable);
    ...
    public ResponseEntity<ApiResponse<ProductDto>> updateProduct(
            ...
            Authentication authentication) {
        CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
        ProductDto updated = productService.updateProduct(id, request, userDetails.getId());
    ...
    @SecurityRequirement(name = "Bearer Authentication")
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> getPendingProducts(
            @PageableDefault(size = 20, sort = "createdAt", direction = Sort.Direction.ASC) Pageable pageable) {
        Page<ProductSummaryDto> products = productService.getPendingProducts(pageable);
    ...
    @PreAuthorize("hasAnyRole('STAFF', 'ADMIN')")
    @SecurityRequirement(name = "Bearer Authentication")
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> getAllProductsForAdmin(
            @Parameter(description = "Optional search keyword", example = "laptop")
            @RequestParam(required = false) String keyword,
            @PageableDefault(size = 20, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
        Page<ProductSummaryDto> products;
        if (keyword != null && !keyword.trim().isEmpty()) {
            products = productService.getAllProductsForAdminWithKeyword(keyword, pageable);
        } else {
    ...
    @PreAuthorize("hasAnyRole('STAFF', 'ADMIN')")
    @SecurityRequirement(name = "Bearer Authentication")
    public ResponseEntity<ApiResponse<Page<ProductSummaryDto>>> getProductsByStatus(
            @Parameter(description = "Product status", example = "APPROVED")
        ...
        Page<ProductSummaryDto> products;
        if (keyword != null && !keyword.trim().isEmpty()) {
            products = productService.getProductsByStatusAndKeyword(status, keyword, pageable);
        } else {
    ...
    public ResponseEntity<ApiResponse<ProductDto>> approveProduct(
            ...
            Authentication authentication) {
        CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
        ProductDto approved = productService.approveProduct(id, userDetails.getId());
        return ResponseEntity.ok(ApiResponse.success("Product approved successfully", approved));
    }
    ...
    public ResponseEntity<ApiResponse<ProductDto>> rejectProduct(
            ...
            Authentication authentication) {
        CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
        ProductDto rejected = productService.rejectProduct(id, request.getReason(), userDetails.getId());
        return ResponseEntity.ok(ApiResponse.success("Product rejected successfully", rejected));
    }
}